...
>(({ className, align = 'end', children, ...props }, ref) => {
  ...
  return createPortal(
    <div
      ...
      className={cn(
        'fixed z-[9999] min-w-[18rem] overflow-hidden rounded-lg border border-border bg-card shadow-lg',
        className
      )}
      style={{
        top: position.top,
...
>(({ className, children, onSelect, closeOnSelect = true, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'relative flex cursor-pointer select-none items-center rounded-sm px-3 py-2 text-sm outline-none transition-colors text-foreground hover:bg-muted dark:hover:bg-card focus:bg-muted dark:focus:bg-card',
        className
      )}
      onClick={handleClick}
      {...props}
