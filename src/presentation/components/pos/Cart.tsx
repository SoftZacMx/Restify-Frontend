...
export const Cart: React.FC<CartProps> = ({ items, onRemoveItem, readOnly = false, className }) => {
  ...
  return (
    <Card className={`shadow-lg border-0 flex flex-col min-h-0 ${className ?? ''}`} data-testid="cart">
      <CardHeader className="shrink-0 bg-gradient-to-r from-primary/10 to-primary/5 border-b border-border">
      ...
      <CardContent className="p-4 space-y-3 flex-1 min-h-0 overflow-y-auto" data-testid="cart-items">
        {items.map((item) => (
          <div
            key={item.id}
            className="group relative border border-border rounded-xl p-4 space-y-3 bg-card hover:shadow-md transition-all duration-200 hover:border-primary/30"
