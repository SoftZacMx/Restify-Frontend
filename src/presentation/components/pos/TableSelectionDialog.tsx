...
export const TableSelectionDialog: React.FC<TableSelectionDialogProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[90vw] max-w-4xl max-h-[90vh] flex flex-col bg-white dark:bg-background border-slate-200 dark:border-border p-0 overflow-hidden">
        <DialogHeader className="relative px-6 pt-6 pb-4 border-b border-slate-200 dark:border-border shrink-0">
        ...
        <div className="p-6 overflow-y-auto">
          {isLoading ? (
            <div className="text-center py-12 text-slate-500 dark:text-muted-foreground">Cargando ubicaciones...</div>
          ) : error ? (
            <div className="text-center py-12 text-red-600 dark:text-red-400">{error}</div>
          ) : tables.length === 0 ? (
            <div className="text-center py-12 text-slate-500 dark:text-muted-foreground">No hay ubicaciones disponibles</div>
          ) : (
            <TableSelector
              tables={tables}
