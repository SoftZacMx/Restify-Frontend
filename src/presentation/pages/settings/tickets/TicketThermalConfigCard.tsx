...
const fieldLabelClass =
  'text-[10px] font-semibold uppercase tracking-wider text-muted-foreground block mb-1.5';
  ...
  return (
    <div
      className={cn(
        ...
        nested
          ? 'border-border/90 dark:border-border/60 bg-muted/50 dark:bg-background/40'
    ...
    >
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          'flex w-full items-center gap-3 text-left transition-colors hover:bg-muted/80 dark:hover:bg-card/40',
          nested ? 'p-2.5' : 'p-3.5'
        )}
      >
        {Icon ? (
...
export const TicketThermalConfigCard: React.FC<TicketThermalConfigCardProps> = ({
  ...
  return (
    <Card className="rounded-xl border-border bg-card dark:bg-background/30 shadow-sm overflow-hidden">
      <CardHeader className="space-y-1 pb-2">
        <CardTitle className="text-lg text-foreground">Configuración de ticket</CardTitle>
